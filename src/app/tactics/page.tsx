...
const W = 105
const H = 68
const SPEED = 20        // field units per second for animation
const ASSIGN_RADIUS = 8 // snap radius for run-line → player assignment

function segLen(a: { x: number; y: number }, b: { x: number; y: number }) {
...
function interpolateAlongPath(pts: { x: number; y: number }[], t: number) {
...
export default function TacticsPage() {
  ...
  const [activePassLineIdx, setActivePassLineIdx] = useState(-1)
  ...
  useEffect(() => {
    ...
    sb.from("tactics_plays")
      .select("*")
      .eq("academy_id", academyId)
      .order("created_at", { ascending: false })
      .then(({ data }: { data: any[] | null }) => {
        if (data) setPlays(data.map((r: any) => ({
          id: r.id,
          name: r.name,
  ...
  // Build sequential paths per entity:
  // - Ball: all pass lines concatenated in drawing order
  // - Each player: nearest run line (by proximity at line start)
  function buildEntityPaths() {
    const ball = markers.find(m => m.team === "ball")
    type EntityPath = { markerId: string; points: { x: number; y: number }[]; passLineIndices?: number[] }
    const entities: EntityPath[] = []

    // Ball path: concatenate ALL pass lines in drawing order
    if (ball) {
      const passPoints: { x: number; y: number }[] = []
      const passLineIndices: number[] = []
      lines.forEach((line, li) => {
        if (line.type !== "pass" || line.points.length < 2) return
        passPoints.push(...line.points)
        passLineIndices.push(li)
      })
      if (passPoints.length >= 2) entities.push({ markerId: ball.id, points: passPoints, passLineIndices })
    }

    // Player run paths: each run line → nearest player marker
    const usedMarkers = new Set<string>()
    const playerPts = new Map<string, { x: number; y: number }[]>()
    lines.forEach(line => {
      if (line.type !== "run" || line.points.length < 2) return
      ...
      if (nearest) {
        ...
        playerPts.set(nearest.id, [...(playerPts.get(nearest.id) ?? []), ...line.points])
      }
    })
    playerPts.forEach((points, markerId) => {
      if (points.length >= 2) entities.push({ markerId, points })
    })
  ...
  const stopAnimation = useCallback(() => {
    ...
    setAnimProgress(0)
    setAnimPositions({})
    setActivePassLineIdx(-1)
  }, [])

  function togglePlay() {
    if (isPlaying) { stopAnimation(); return }
    const entityPaths = buildEntityPaths()
    if (entityPaths.length === 0) return

    const maxLen = Math.max(...entityPaths.map(e => pathLength(e.points)))
    if (maxLen === 0) return
    const duration = (maxLen / SPEED) * 1000

    // Pre-compute t-thresholds for which pass line is "active" at any point
    const ballEntity = entityPaths.find(e => e.passLineIndices !== undefined)
    const passThresholds: { lineIdx: number; tStart: number; tEnd: number }[] = []
    if (ballEntity?.passLineIndices) {
      const total = pathLength(ballEntity.points)
      let acc = 0
      ballEntity.passLineIndices.forEach(lineIdx => {
        const linePts = lines[lineIdx]?.points ?? []
        const len = pathLength(linePts)
        const tStart = total > 0 ? acc / total : 0
        acc += len
        passThresholds.push({ lineIdx, tStart, tEnd: total > 0 ? acc / total : 1 })
      })
    }
    ...
    function tick(now: number) {
      ...
      for (const { markerId, points } of entityPaths) {
        positions[markerId] = interpolateAlongPath(points, t)
      }
      setAnimPositions(positions)
      if (passThresholds.length > 0) {
        const active = [...passThresholds].reverse().find(th => t >= th.tStart)
        setActivePassLineIdx(active?.lineIdx ?? passThresholds[0].lineIdx)
      }
      if (t < 1) {
  ...
  async function deletePlay(id: string) {
    ...
    await (supabase as any).from("tactics_plays").delete().eq("id", id)
    setPlays(ps => ps.filter(p => p.id !== id))
  }

  const entityPaths = buildEntityPaths()
  const hasAnimatable = entityPaths.length > 0
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 md:p-6 border border-slate-100 dark:border-slate-800 mb-6">
          ...
          {isCoach && (
            <div className="flex items-center gap-2 mb-4 flex-wrap">
              ...
              <button
                ...
                title={isPlaying ? "Detener" : !hasAnimatable ? "Dibuja líneas desde jugadores o el balón" : `Animar jugada (${entityPaths.length} elemento${entityPaths.length !== 1 ? "s" : ""})`}
          ...
          {isCoach && !isPlaying && (
            <p className="text-xs text-slate-400 dark:text-slate-500 mb-3">
              ...
              {hasAnimatable && (
                <span className="ml-2 text-emerald-600 dark:text-emerald-400 font-medium">· {entityPaths.length} elemento{entityPaths.length !== 1 ? "s" : ""} listo{entityPaths.length !== 1 ? "s" : ""} para animar.</span>
              )}
            </p>
          )}
          ...
          <div className="relative w-full rounded-xl overflow-hidden bg-gradient-to-b from-emerald-600 to-emerald-700 select-none touch-none" style={{ aspectRatio: "105/68", maxHeight: "380px" }}>
            ...
            >
              <defs>
                ...
                <marker id="arr-active" markerWidth="4" markerHeight="4" refX="3" refY="2" orient="auto" markerUnits="userSpaceOnUse">
              ...
              {/* Lines — active pass line highlights white during animation */}
              {lines.map((l, li) => {
                const isActiveLine = isPlaying && l.type === "pass" && activePassLineIdx === li
                return (
                  <polyline
                    key={l.id}
                    points={l.points.map(p => `${p.x},${p.y}`).join(" ")}
                    fill="none"
                    stroke={isActiveLine ? "#ffffff" : l.type === "pass" ? "#38BDF8" : "#FBBF24"}
                    strokeWidth={isActiveLine ? 1.1 : 0.7}
                    strokeOpacity={isPlaying && !isActiveLine ? 0.35 : 1}
                    strokeDasharray={l.type === "pass" && !isActiveLine ? "1.4 1" : undefined}
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    markerEnd={isActiveLine ? "url(#arr-active)" : l.type === "pass" ? "url(#arr-pass)" : "url(#arr-run)"}
