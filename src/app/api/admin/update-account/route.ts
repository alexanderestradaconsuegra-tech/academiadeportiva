...
export async function POST(req: NextRequest) {
  ...
  if (!player_id || (!email && !password)) {
    return NextResponse.json({ error: "Jugador y al menos un cambio (correo o contraseña) son requeridos." }, { status: 400 })
  }

  const { data: targetProfile, error: targetProfileError } = await admin
    .from("profiles").select("id, academy_id").eq("player_id", player_id).single()
  if (targetProfileError || !targetProfile) {
    return NextResponse.json({ error: "Este jugador no tiene acceso creado." }, { status: 404 })
  }
